...
with open('comma_sep_data_to_tag.csv', mode='w') as csv_file:
            ...
            for paragraph in full_text_list:
                ...
                for sent in sentences:
                    if any(word.lower() in sent for word in keep_sentence_if_contains):
                        ...
                        # print('-------------------------------------------')
                        sent = re.sub("[\[].*?[\]]", "", sent)
                        if sent.strip() not in duplicate_list:
                            subsent = re.split('; |, |-', sent)
                            for part in subsent:
                                part = re.sub('\"', '', part)
                                if len(part.split()) >= 3:
                                    writer.writerow([" ".join(part.split()), "", "", "", search_tracker[i]])
                                    duplicate_list.append(part.strip())
        i += 1
